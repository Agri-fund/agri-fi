...
export const Header: React.FC<HeaderProps> = ({ user, onLogout }) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-slate-100 px-4 md:px-6 h-16 flex items-center justify-between">
        ...
        >
          <div className="w-6 h-5 relative flex flex-col justify-between items-center">
            ...
            <span
              className={`w-full h-0.5 bg-slate-900 rounded-full origin-center transition-transform duration-300 ${
                isOpen ? 'translate-y-[9px] rotate-45' : ''
              }`}
            />
            {/* Middle bar: fades out when open */}
            <span
              className={`w-full h-0.5 bg-slate-900 rounded-full transition-opacity duration-200 ${
      ...
      {/* Drawer Panel */}
      <div
        className={`fixed right-0 top-0 bottom-0 z-[70] w-[80%] max-w-sm bg-white shadow-2xl md:hidden flex flex-col transition-transform duration-300 ease-in-out ${

...
export class FarmerCreditScoringService {
  ...
  public calculateScoreFromFactors(factors: ScoreFactorsInput): number {
    const repaymentComponent = Math.min(Math.max(factors.onTimeRepaymentRate, 0), 1) * 0.35;
    const completionComponent = Math.min(Math.max(factors.dealCompletionRate, 0), 1) * 0.20;
    const defaultInverted = Math.max(0, 1 - Math.min(Math.max(factors.dealDefaultRate, 0), 1));
    const defaultComponent = defaultInverted * 0.20;
    const milestoneComponent = Math.min(Math.max(factors.shipmentMilestoneComplianceRate, 0), 1) * 0.15;
    
    // Normalize KYC age: 365 days or more gives full 1.0
    const kycNormalized = Math.min(Math.max(factors.kycVerificationAgeDays, 0) / 365, 1);
    const kycComponent = kycNormalized * 0.10;

    const weightedScore = repaymentComponent + completionComponent + defaultComponent + milestoneComponent + kycComponent;
  ...
  async computeScore(userId: string, reason: string = 'Periodic recalculation', customFactors?: ScoreFactorsInput): Promise<{
    score: number;
    ...
    this.logger.log(`Credit score updated for user ${userId}: ${score} (Tier max: $${maxDealSizeUsdc})`);

    return { score, maxDealSizeUsdc, factors };
  }

  async manualOverride(adminId: string, userId: string, score: number, reason: string): Promise<FarmerCreditScoreHistory> {
    const user = await this.userRepository.findOne({ where: { id: userId } });
    if (!user) {
      throw new NotFoundException(`User with ID ${userId} not found`);
    }
